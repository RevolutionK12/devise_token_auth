...
Make extra sure that the `Access-Control-Expose-Headers` includes `Authorization`,  and `client` (as is set in the example above by the`:expose` param). If your client experiences erroneous 401 responses, this is likely the cause.
...
| param | description |
|---|---|
| **`Authorization`** | This serves as the user's password for each request. A hashed version of this value is stored in the database for later comparison. This value should be changed on each request. |
